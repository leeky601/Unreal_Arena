...
bool UBTTask_RotateToFaceTarget::HasReachedAnglePrecision(APawn* OwningPawn, AActor* TargetActor) const
{
	FVector OwnerForward = OwningPawn->GetActorForwardVector();
	FVector OwnerToTargetNormalized = (TargetActor->GetActorLocation() - OwningPawn->GetActorLocation()).GetSafeNormal();

	const float DotProduct = FVector::DotProduct(OwnerForward, OwnerToTargetNormalized);
